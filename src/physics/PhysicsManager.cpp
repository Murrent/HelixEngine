...
bool PhysicsManager::checkArea(sf::Vector2f topLeft, sf::Vector2f btmRight, sf::Vector2u tileOccupation,
                               sf::Vector2i &tile) {
...
bool PhysicsManager::continousRectCheckXPlayer(RectangleObject &obj, sf::Vector2f dest, sf::Vector2u tileOccupation) {
    if (obj.getVelocity().x == 0.0f) return false;
    const sf::Vector2f &pos = obj.getPosition();
    float size2 = obj.getSize().x * 2.0f;
    if (size2 == 0.0f) return false;
    ...
    for (unsigned int i = 1; i <= xSteps; ++i) {
        ...
        if (positiveX) {
            if (checkRightVel(topLeft, btmRight, tileOccupation, outTile)) {
                if (outTile.y == (int) std::floor(btmRight.y)) {
                    float newBottom =
                            std::floor(btmRight.y) - 0.1f; // Maybe add small offset to make sure you're in the tile
                    float difference = newBottom - btmRight.y;
                    btmRight.y = newBottom;
                    topLeft.y += difference;
                    btmRight.x += 0.1f;
                    topLeft.x += 0.1f;
                    sf::Vector2i outTile2;
                    if (!checkArea(topLeft, btmRight, tileOccupation, outTile2)) {
                        obj.setPosition(sf::Vector2f(pos.x, pos.y + difference)); // Test, this wont work at high speeds
                        return false;
                    }
                }
                obj.setPosition(sf::Vector2f(float(outTile.x) - obj.getSize().x - 0.0001f, pos.y));
...
void PhysicsManager::playerTilemap(RectangleObject &obj) {
    const sf::Vector2f &pos = obj.getPosition();
    sf::Vector2f nextPos = pos + obj.getVelocity();
    sf::Vector2u occupation = sf::Vector2u(std::ceil(obj.getSize().x * 2.0f) + 1,
                                           std::ceil(obj.getSize().y * 2.0f) + 1);
    ...
    if (!continousRectCheckXPlayer(obj, nextPos, occupation)) {
        obj.setPosition(sf::Vector2f(nextPos.x, obj.getPosition().y));
    } else {
        obj.setVelocity(sf::Vector2f(0.0f, obj.getVelocity().y));
    }
